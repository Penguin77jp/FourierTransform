...
int main() {
	if (false) {
		const std::string imagePath = "assets/penguin_low.jpg";
...
#pragma omp parallel for schedule(dynamic)
			...
			for (int indexY = 0; indexY < sizeY; ++indexY) {
				for (int indexX = 0; indexX < sizeX; ++indexX) {
					const int index = indexY * sizeX * 3 + indexX * 3;
					tmpImage[index + 0] = 255.5 * data_converted[indexX][indexY].real() / max;
					tmpImage[index + 1] = 255.5 * data_converted[indexX][indexY].imag() / max;
					tmpImage[index + 2] = 0;
				}
			}
	...
	if (true) {
		...
		// load fourier image
		std::vector<std::vector<double>> data_resotre;
		std::vector<std::vector<std::complex<double>>> data_converted;
		int sizeY, sizeX;
		{
			int c;
			unsigned char* data = stbi_load("fourier.bmp", &sizeX, &sizeY, &c, 0);
			data_resotre = std::vector<std::vector<double>>(sizeX, std::vector<double>(sizeY));
			data_converted = std::vector<std::vector<std::complex<double>>>(sizeX, std::vector<std::complex<double>>(sizeY));
			for (int y = 0; y < sizeY; ++y) {
				for (int x = 0; x < sizeX; ++x) {
					data_converted[x][y] = std::complex<double>{
						(double)data[y * sizeX * c + x * c] / 255.5,
						(double)data[y * sizeX * c + x * c + 1] / 255.5
					};
				}
			}
		}
		...
		// inv fourier
		{
			std::cout << "compute inv-fourier" << std::endl;
#pragma omp parallel for schedule(dynamic)
			for (int indexY = 0; indexY < sizeY; ++indexY) {
				std::cout << std::format("{} / {}", indexY, sizeY) << std::endl;
				for (int indexX = 0; indexX < sizeX; ++indexX) {
					double result = 0;
					const double freq_res = 1;
					for (int searchY = 0; searchY < sizeY; ++searchY) {
						for (int searchX = 0; searchX < sizeX; ++searchX) {
							double tmpY = 2.0 * std::numbers::pi * searchY * indexY / sizeY;
							double tmpX = 2.0 * std::numbers::pi * searchX * indexX / sizeX;
		...
		std::vector<unsigned char> out_image(sizeX * sizeY * 3, 0);
		{
			...
			for (int indexY = 0; indexY < sizeY; ++indexY) {
				for (int indexX = 0; indexX < sizeX; ++indexX) {
					const int index = indexY * sizeX * 3 + indexX * 3;
					out_image[index + 0] = 255.5 * data_resotre[indexX][indexY];
					out_image[index + 1] = 255.5 * data_resotre[indexX][indexY];
					out_image[index + 2] = 255.5 * data_resotre[indexX][indexY];
				}
			}
